...
const handleProsesPembayaran = async (pesanan) => {
  try {
    // Hitung subtotal, ppn, total berdasarkan item
    const subtotal = pesanan.items.reduce(
    ...
    const res = await fetch(
      `http://localhost:5000/pesanan/${pesanan._id}/bayar`,
      {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ subtotal, ppn, total }),
      }
    );
    ...
    if (res.ok) {
      ...
      const transaksiId = `TRX-${Date.now()}-${uuidv4()
        .slice(0, 6)
        .toUpperCase()}`;
      ...
      setDataPesanan((prev) =>
        prev.map((p) =>
          p._id === pesanan._id
            ? { ...p, dibayar: true, tanggal, subtotal, ppn, total }
            : p
        )
      );
    }
  ...
  return (
    <div className="d-flex flex-column min-vh-100">
      ...
      <main className="flex-grow-1 px-3 py-4">
        {tabAktif === "pesanan" && (
          <>
            <h5 className="fw-bold mb-3" style={{ color: "#C99D4D" }}>
            ...
            <div className="row g-3">
              ...
              <div className="col-md-6">
                <div className="card shadow-sm p-3">
                  <p className="mb-1">Total Pendapatan</p>
                  <h4 className="text-dark fw-bold">
                ...
                pesananBelumLunas.map((pesanan, idx) => (
                  ...
                  >
                    ...
                    <button
                      ...
                      style={{ backgroundColor: "#C99D4D", color: "white" }}
                      onClick={() => handleProsesPembayaran(pesanan)}
                    >
                      Proses Pembayaran
                    </button>
                  </div>
